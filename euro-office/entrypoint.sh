...
# WOPI keypair
#
# ПАТЧ 1: openssl genrsa -traditional генерирует PKCS#1 (BEGIN RSA PRIVATE KEY).
#   Оригинальный entrypoint использует genpkey → PKCS#8 (BEGIN PRIVATE KEY).
#   Node.js crypto в Euro-Office не поддерживает PKCS#8 → DECODER::unsupported.
#
# ПАТЧ 2: WOPI_PRIVATE_KEY_DATA=$(cat ...) — реальные переносы строк в PEM.
#   Оригинал использует awk '{printf "%s\\n"}' — literal \n в строке.
#   Node.js createPrivateKey падает на literal \n → тот же DECODER::unsupported.
#
# ПАТЧ 3: Миграция существующих ключей PKCS#8 → PKCS#1.
#   Нужна при первом запуске на volume с ключом от предыдущей (непатченной) версии.
# --------------------------------------------------------------------
...
if [ "$WOPI_ENABLED" = "true" ]; then
  if [ ! -f "$WOPI_PRIVATE_KEY" ]; then
    ...
    openssl genrsa -traditional -out "$WOPI_PRIVATE_KEY" 2048 >/dev/null 2>&1
  fi
  chmod 600 "$WOPI_PRIVATE_KEY" 2>/dev/null || true
  # Миграция PKCS#8 → PKCS#1 если ключ был создан без патча
  if head -1 "$WOPI_PRIVATE_KEY" 2>/dev/null | grep -q "BEGIN PRIVATE KEY"; then
    echo "Migrating WOPI key PKCS#8 -> PKCS#1..."
    openssl rsa -traditional -in "$WOPI_PRIVATE_KEY" -out "${WOPI_PRIVATE_KEY}.tmp" >/dev/null 2>&1 \
      && mv "${WOPI_PRIVATE_KEY}.tmp" "$WOPI_PRIVATE_KEY" && rm -f "$WOPI_PUBLIC_KEY"
  fi
  ...
  # cat даёт реальные переносы строк — нужно для Node.js crypto
